...
struct BfsData
{
  ...
  int d_distFromSource;

  BfsData(EqualityNodeId nodeId = null_id,
          EqualityEdgeId edgeId = null_edge,
          int dist = 0)
      : d_nodeId(nodeId), d_edgeId(edgeId), d_distFromSource(dist)
  {
  }
};
...
EqualityEngine::~EqualityEngine() { free(d_triggerDatabase); }
...
EqualityNodeId EqualityEngine::newApplicationNode(TNode original,
                                                  EqualityNodeId t1,
                                                  EqualityNodeId t2,
                                                  FunctionApplicationType type)
{
  ...
  else
  {
    ...
    enqueue(MergeCandidate(
        funId, find->second, MERGED_THROUGH_CONGRUENCE, TNode::null()));
  }
...
void EqualityEngine::addTermInternal(TNode t, bool isOperator)
{
  ...
  else if (t.getNumChildren() > 0 && d_congruenceKinds[tk])
  {
    ...
    for (unsigned i = 0; i < t.getNumChildren(); ++i)
    {
      ...
      // Add the application
      result = newApplicationNode(
          t, result, tiId, isInterpreted ? APP_INTERPRETED : APP_UNINTERPRETED);
    }
    d_isInternal[result] = false;
...
bool EqualityEngine::merge(EqualityNode& class1,
                           EqualityNode& class2,
                           std::vector<TriggerId>& triggersFired)
{
  ...
  {
    getDisequalities(!class1isConstant,
                     class2Id,
                     class1OnlyTags,
                     class2disequalitiesToNotify);
    getDisequalities(!class2isConstant,
                     class1Id,
                     class2OnlyTags,
  ...
  if (!d_isEquality[class2Id])
  {
    ...
    do
    {
      ...
      while (currentUseId != null_uselist_id)
      {
        ...
        ApplicationIdsMap::iterator find =
            d_applicationLookup.find(funNormalized);
        if (find != d_applicationLookup.end())
        {
          // Applications fun and the funNormalized can be merged due to
          // congruence
          if (getEqualityNode(funId).getFind()
              != getEqualityNode(find->second).getFind())
          {
            enqueue(MergeCandidate(
                funId, find->second, MERGED_THROUGH_CONGRUENCE, TNode::null()));
          }
        ...
        {
          // There is no representative, so we can add one, we remove this when
          // backtracking
          storeApplicationLookup(funNormalized, funId);
        }
  ...
  if (!propagateTriggerTermDisequalities(
          class1OnlyTags, class1triggerRef, class2disequalitiesToNotify))
  {
    return false;
  }
  if (!propagateTriggerTermDisequalities(
          class2OnlyTags, class2triggerRef, class1disequalitiesToNotify))
  {
    return false;
  }
  ...
  if (class2triggerRef != +null_set_id)
  {
    if (class1triggerRef == +null_set_id)
    {
      ...
      // Add it to the list for backtracking
      d_triggerTermSetUpdates.push_back(
          TriggerSetUpdate(class1Id, +null_set_id));
      d_triggerTermSetUpdatesSize = d_triggerTermSetUpdatesSize + 1;
    }
    else
    {
      ...
      if (class1triggers.d_tags != class2triggers.d_tags)
      {
        // Add it to the list for backtracking
        d_triggerTermSetUpdates.push_back(
            TriggerSetUpdate(class1Id, class1triggerRef));
        d_triggerTermSetUpdatesSize = d_triggerTermSetUpdatesSize + 1;
        // Mark the the new set as a trigger
        d_nodeIndividualTrigger[class1Id] =
            newTriggerTermSet(newSetTags, newSetTriggers, newSetTriggersSize);
      }
    }
  }
...
void EqualityEngine::undoMerge(EqualityNode& class1,
                               EqualityNode& class2,
...
void EqualityEngine::backtrack()
{
  ...
  if (d_assertedEqualitiesCount < d_assertedEqualities.size() || expectedEdgesCount < d_equalityEdges.size()) {
    ...
    for (int i = (int)d_assertedEqualities.size() - 1,
             i_end = (int)d_assertedEqualitiesCount;
         i >= i_end;
         --i)
    {
    ...
    for (int i = (int)d_equalityEdges.size() - 2; i >= (int)expectedEdgesCount; i -= 2) {
      ...
      EqualityNodeId node2 = edge2.getNodeId();
  ...
  if (d_triggerTermSetUpdates.size() > d_triggerTermSetUpdatesSize)
  {
    ...
    for (int i = d_triggerTermSetUpdates.size() - 1,
             i_end = d_triggerTermSetUpdatesSize;
         i >= i_end;
         --i)
    {
  ...
  if (d_equalityTriggers.size() > d_equalityTriggersCount)
  {
    ...
    for (int i = d_equalityTriggers.size() - 1, i_end = d_equalityTriggersCount;
         i >= i_end;
         --i)
    {
  ...
  {
    for (int i = d_applicationLookups.size() - 1,
             i_end = (int)d_applicationLookupsCount;
         i >= i_end;
         --i)
    {
  ...
  {
    for (int i = d_subtermEvaluates.size() - 1,
             i_end = (int)d_subtermEvaluatesSize;
         i >= i_end;
         --i)
    {
  ...
  {
    // Go down the nodes, check the application nodes and remove them from
    // use-lists
  ...
  {
    for (int i = d_deducedDisequalities.size() - 1,
             i_end = (int)d_deducedDisequalitiesSize;
         i >= i_end;
         --i)
    {
                                     ...
                                     ExplainAlgorithm algo)
{
  ...
  if (polarity) {
    ...
    if (eqp)
    {
      ...
      {
        // Corner case where this is actually a disequality between two
        // constants
        Trace("pf::ee")
...
int EqualityEngine::shortestPath(EqualityNodeId start,
                                 EqualityNodeId end,
                                 uint32_t maxLevel,
                                 const std::vector<int>& edgeWeights) const
{
  ...
  std::priority_queue<BfsData, std::vector<BfsData>, decltype(compare)>
      priorityQueue(compare);
  std::unordered_set<EqualityNodeId> seen;

  priorityQueue.push(BfsData(start, null_edge, 0));

  while (true)
  {
    BfsData current = priorityQueue.top();
    priorityQueue.pop();
    EqualityNodeId currentNode = current.d_nodeId;
    int currentDist = current.d_distFromSource;
         ...
         edge = d_equalityEdges[edge].getNext())
    {
      ...
      if (edgeWeights.empty())
        weight = d_equalityEdges[edge].isRedundant()
...
void EqualityEngine::computeTreeOptWeights()
{
  ...
  while(true) {
    ...
    // When we hit the fixed point, we can stop
    if (!changed) break;
  }
}
    ...
    ExplainAlgorithm algo)
{
  ...
  else
  {
    ...
    getExplanationImpl(
        t1Id, t2Id, 10, level, d_greedyEdgeWeights, equalities, cache, eqp, algo);
  }
}
      ...
      ExplainAlgorithm algo)
{
  ...
  // Find a path from t1 to t2 in the graph
  priorityQueue.push(BfsData(t1Id, null_edge, 0));
  while (true)
  {
    // The next node to visit
    BfsData current = priorityQueue.top();
    priorityQueue.pop();
    EqualityNodeId currentNodeId = current.d_nodeId;
    int currentDist = current.d_distFromSource;

    // Because of the way we update the distance, there might be duplicate
    // entries in the priority queue. So, if this node was already seen, we
    // skip it
    if (traversed.count(currentNodeId))
    {
      continue;
    }
    traversed[currentNodeId] = current.d_edgeId;

    Trace("equality") << d_name << "::eq::getExplanation(): currentNode = {"
                      << currentNodeId << "} " << d_nodes[currentNodeId]
                      << std::endl;
    ...
    if (TraceIsOn("equality")) {
      ...
      Trace("equality") << d_name << "::eq::getExplanation(): edges =  "
                        << edgesToString(currentEdgeId) << std::endl;
    }

    while (currentEdgeId != null_edge)
    {
      // Get the edge
      ...
      if (!isBackEdge && !isForbidden)
      {
        Trace("equality") << d_name
                          ...
                          << currentNodeId << "} " << d_nodes[currentNodeId]
                          << ", {" << d_equalityEdges[currentEdgeId].getNodeId()
                          << "} "
                          << d_nodes[d_equalityEdges[currentEdgeId].getNodeId()]
                          << ")" << std::endl;
        ...
        if (d_equalityEdges[currentEdgeId].getNodeId() == t2Id)
        {
          ...
          do {
            ...
            d_stats.d_redundantExplanationSize +=
                (int)currentEdge.isRedundant();

            // The current node
            currentNodeId = d_equalityEdges[currentEdgeId ^ 1].getNodeId();
            EqualityNodeId edgeNode = currentEdge.getNodeId();

            MergeReasonType reasonType =
                static_cast<MergeReasonType>(currentEdge.getReasonType());
            Node reason = currentEdge.getReason();

            Trace("equality")
                << d_name
                << "::eq::getExplanation(): currentEdge = " << currentEdgeId
                << ", currentNode = " << currentNodeId << std::endl;
            Trace("equality")
            ...
            Trace("equality")
                ...
                << currentNodeId << "} " << d_nodes[currentNodeId] << ", {"
                << currentEdge.getNodeId() << "} "
                << d_nodes[currentEdge.getNodeId()] << ")" << std::endl;
            Trace("equality")
                << d_name
            ...
            // Go to the previous
            currentEdgeId = traversed[currentNodeId];
        ...
        priorityQueue.push(BfsData(d_equalityEdges[currentEdgeId].getNodeId(),
                                   currentEdgeId,
                                   newDist));
      }
...
void EqualityEngine::addTriggerEqualityInternal(TNode t1,
                                                TNode t2,
                                                TNode trigger,
                                                bool polarity)
{
  ...
  // We will attach it to the class representative, since then we know how to
  // backtrack it
...
void EqualityEngine::processEvaluationQueue()
{
  ...
  while (!d_evaluationQueue.empty())
  {
    ...
    enqueue(MergeCandidate(
        id, nodeEvaluatedId, MERGED_THROUGH_CONSTANTS, TNode::null()));
  }
...
void EqualityEngine::propagate()
{
  ...
  while (!d_propagationQueue.empty() || !d_evaluationQueue.empty())
  {
    ...
    {
      // We always merge into the bigger class to reduce the amount of
      // traversing we need to do
      mergeInto = t2classId;
    }
    ...
    if (!d_done)
    {
      for (size_t trigger_i = 0, trigger_end = triggers.size();
           trigger_i < trigger_end && !d_done;
           ++trigger_i)
      {
        const TriggerInfo& triggerInfo =
            d_equalityTriggersOriginal[triggers[trigger_i]];
        if (triggerInfo.d_trigger.getKind() == Kind::EQUAL)
        {
          ...
          if (!triggerInfo.d_polarity)
          {
            ...
            // We use the THEORY_LAST as a marker for "marked as propagated,
            // reasons stored". This tag is added to an internal theories set
            // that is only inserted in, so this is safe. Had we iterated over,
            // or done other set operations this might be dangerous.
            if (!hasPropagatedDisequality(THEORY_LAST, lhsId, rhsId))
            {
              if (!hasPropagatedDisequality(lhsId, rhsId))
              {
                d_deducedDisequalityReasons.push_back(
                    EqualityPair(original, d_falseId));
              }
...
bool EqualityEngine::areDisequal(TNode t1, TNode t2, bool ensureProof) const
{
      ...
      && t1ClassId != t2ClassId)
  {
    if (ensureProof)
    {
      nonConst->d_deducedDisequalityReasons.push_back(
          EqualityPair(t1Id, t1ClassId));
      nonConst->d_deducedDisequalityReasons.push_back(
          EqualityPair(t2Id, t2ClassId));
  ...
  ApplicationIdsMap::const_iterator find =
      d_applicationLookup.find(eqNormalized);
  if (find != d_applicationLookup.end())
  {
    if (getEqualityNode(find->second).getFind()
        == getEqualityNode(d_falseId).getFind())
    {
      if (ensureProof)
      {
        ...
        nonConst->d_deducedDisequalityReasons.push_back(
            EqualityPair(find->second, d_falseId));
        nonConst->d_deducedDisequalityReasons.push_back(
...
void EqualityEngine::addTriggerTerm(TNode t, TheoryId tag)
{
  ...
  if (triggerSetRef != +null_set_id
      && getTriggerTermSet(triggerSetRef).hasTrigger(tag))
  {
    // If the term already is in the equivalence class that a tagged
    // representative, just notify
  ...
  {
    // Check for disequalities by going through the equivalence class looking
    // for equalities in the uselists that have been asserted to false. All the
    // representatives appearing on the other side of such disequalities, that
    // have the tag on, are put in a set.
    TaggedEqualitiesSet disequalitiesToNotify;
    TheoryIdSet tags = TheoryIdSetUtil::setInsert(tag);
    getDisequalities(
        !d_isConstant[classId], classId, tags, disequalitiesToNotify);
    ...
    d_triggerTermSetUpdatesSize = d_triggerTermSetUpdatesSize + 1;
    // Mark the the new set as a trigger
    d_nodeIndividualTrigger[classId] = triggerSetRef =
        newTriggerTermSet(newSetTags, newSetTriggers, newSetTriggersSize);
    ...
    propagateTriggerTermDisequalities(
        tags, triggerSetRef, disequalitiesToNotify);
  }
}
...
bool EqualityEngine::isTriggerTerm(TNode t, TheoryId tag) const
{
  ...
  TriggerTermSetRef triggerSetRef = d_nodeIndividualTrigger[classId];
  return triggerSetRef != +null_set_id
         && getTriggerTermSet(triggerSetRef).hasTrigger(tag);
}
...
TNode EqualityEngine::getTriggerTermRepresentative(TNode t, TheoryId tag) const
{
  ...
  EqualityNodeId classId = getEqualityNode(t).getFind();
  const TriggerTermSet& triggerSet =
      getTriggerTermSet(d_nodeIndividualTrigger[classId]);
  unsigned i = 0;
...
void EqualityEngine::storeApplicationLookup(FunctionApplication& funNormalized,
                                            EqualityNodeId funId)
{
  ...
  if (funNormalized.isEquality())
  {
    ...
    {
      enqueue(MergeCandidate(
          funId, d_trueId, MERGED_THROUGH_REFLEXIVITY, TNode::null()));
    }
    else if (d_isConstant[funNormalized.d_a] && d_isConstant[funNormalized.d_b])
    {
      enqueue(MergeCandidate(
          funId, d_falseId, MERGED_THROUGH_CONSTANTS, TNode::null()));
    }
  }
}
    ...
    unsigned newSetTriggersSize)
{
  ...
  size_t size =
      sizeof(TriggerTermSet) + newSetTriggersSize * sizeof(EqualityNodeId);
  // Align the size
  ...
  if (d_triggerDatabaseSize + size > d_triggerDatabaseAllocatedSize)
  {
    ...
    d_triggerDatabase =
        (char*)realloc(d_triggerDatabase, d_triggerDatabaseAllocatedSize);
  }
  // New reference
...
bool EqualityEngine::hasPropagatedDisequality(EqualityNodeId lhsId,
                                              EqualityNodeId rhsId) const
{
  EqualityPair eq(lhsId, rhsId);
  bool propagated =
      d_propagatedDisequalities.find(eq) != d_propagatedDisequalities.end();
#ifdef CVC5_ASSERTIONS
  bool stored =
      d_disequalityReasonsMap.find(eq) != d_disequalityReasonsMap.end();
...
bool EqualityEngine::hasPropagatedDisequality(TheoryId tag,
                                              EqualityNodeId lhsId,
                                              EqualityNodeId rhsId) const
{
  EqualityPair eq(lhsId, rhsId);

  PropagatedDisequalitiesMap::const_iterator it =
      d_propagatedDisequalities.find(eq);
...
void EqualityEngine::storePropagatedDisequality(TheoryId tag,
                                                EqualityNodeId lhsId,
                                                EqualityNodeId rhsId)
{
  ...
  PropagatedDisequalitiesMap::const_iterator find =
      d_propagatedDisequalities.find(pair1);
  ...
  if (d_deducedDisequalityReasons.size() > d_deducedDisequalityReasonsSize)
  {
    ...
    DisequalityReasonRef ref(d_deducedDisequalityReasonsSize,
                             d_deducedDisequalityReasons.size());
#ifdef CVC5_ASSERTIONS
                                      ...
                                      TaggedEqualitiesSet& out)
{
  ...
  Assert(out.size() == 0);
  // The class we are looking for, shouldn't have any of the tags we are looking
  // for already set
  ...
  do
  {
    ...
    while (currentUseId != null_uselist_id)
    {
      ...
      if (fun.isEquality()
          && getEqualityNode(funId).getFind()
                 == getEqualityNode(d_false).getFind())
      {
        ...
        if (alreadyVisited.count(toCompareRep) == 0)
        {
          ...
          // Get the trigger set
          TriggerTermSetRef toCompareTriggerSetRef =
              d_nodeIndividualTrigger[toCompareRep];
          // We only care if we're not both constants and there are trigger
          // terms in the other class
          if ((allowConstants || !d_isConstant[toCompareRep])
              && toCompareTriggerSetRef != null_set_id)
          {
            // Tags of the other gey
            TriggerTermSet& toCompareTriggerSet =
                getTriggerTermSet(toCompareTriggerSetRef);
            // We only care if there are things in inputTags that is also in
            // toCompareTags
    ...
    const TaggedEqualitiesSet& disequalitiesToNotify)
{
  ...
  for (; !d_done && it != it_end; ++it)
  {
    ...
    if (getEqualityNode(toCompare).getFind()
        == getEqualityNode(myCompare).getFind())
    {
      // We're propagating a != a, which means we're inconsistent, just bail and
      // let it go into a regular conflict
      return !d_done;
    }
